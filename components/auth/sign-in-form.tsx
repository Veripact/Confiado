...
export function SignInForm() {
  ...
  const { login, isLoading: web3Loading, user } = useWeb3Auth()
  ...
  const handleWeb3AuthSignIn = async () => {
    try {
      ...
      // Update user in store with Web3Auth user info
      if (user) {
        updateUser({
          id: user.verifierId || "web3auth-user",
          name: user.name || "Web3Auth User",
          email: user.email || "",
...
{isLoading || web3Loading ? "Connecting..." : "Continue with Web3Auth"}
      </Button>
